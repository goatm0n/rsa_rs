...
//    
pub fn decrypt_utf8(encrypted_utf8:&Vec<usize>, private_key:&PrivateKey) -> Vec<u8> {
    let exp:&usize = private_key.private_exponent();
    let modulus:&usize = private_key.modulus();
    let mut decrypted_byte:usize;
    let mut utf8: u8;
...
//    - return string
//
pub fn decrypt_string(encrypted_utf8:&Vec<usize>, private_key:&PrivateKey) -> String {
...
pub fn test() {
    ...
    let encrypted_string:Vec<usize> = encrypt_string(&s, key_pair.public_key());
